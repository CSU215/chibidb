...
> 最后更新：P3 元数据库 `chibi_meta`（databases/users 系统表）+ `CREATE/DROP USER`；多库接入前端（默认库 `main`）；345 tests 全绿、clippy 零警告。
...
| `instance.rs` | 单实例多库：数据根 `<db>/` + 系统库 `chibi_meta/`（真实 Database，`databases`/`users` 表；口令加盐 SHA-256）；`execute_with` 顶层入口（拦截 DB/用户级语句，其余路由到 current_db） | `Instance::open` / `execute_with` / `create_user` / `authenticate` |
...
提交基线：`3f6ed3e feat: support CREATE USER ... IDENTIFIED BY and DROP USER`（HEAD）。
...
| P3 | 单实例多库 + 系统元数据库 + 用户/权限 | 🟡 多库 + 前端接入 + 系统库 `chibi_meta`（`databases`/`users` 表）+ `CREATE/DROP USER`（`e99c5ef`…`3f6ed3e`）；认证尚未在协议层强制、权限/GRANT 待做 |
