...
- 换入侧无逐页流水日志（**换出侧已实现**：`observability.eviction_log` 每次淘汰一行，`observability.cache_stats` 在 checkpoint/退出输出命中与换出计数快照，见 `docs/os_storage.md` §6.10）；无空闲页链表，页只追加不释放
...
提交基线：`aacb920 feat: buffer-pool lookup counters and reproducible perf probes`（HEAD）。
...
| P5 | 执行模型：基准 → 火山算子 → Chunk | ✅ SELECT 全走算子（单/多表、`HashJoin`、分组聚合、DISTINCT、ORDER、LIMIT、UNION、视图、相关/不相关子查询）；物化 SELECT 主干已删除。Chunk 层首批落地：列式 `Chunk`/`Column`（`src/exec/chunk.rs`，`CHUNK_ROWS=1024`）+ `PhysicalOperator::next_chunk` 桥接 + `RowScanner::next_batch`；原生批处理 `TableScan`/`IndexScan`/`Filter`/`Project` + 全局聚合列式核（`chunk_global_aggregate`），由 `execution.mode = "chunk"` 启用，其余算子回退行路径；`tests/chunk.rs` 差分保证两模式结果一致。Sort/Join 向量化待下一轮 |
...
**正确性硬化（`tests/engine_equivalence.rs`）**：差分随机测试——同一确定性语句脚本（固定种子 xorshift，混合插入/重复主键/更新/删除/有序扫描/主键点查/范围扫描/聚合）分别打到 `engine=heap` 与 `engine=lsm`，逐步比对语句结果与全表快照；第二例在中途交替 flush、干净重开（SSTable+MANIFEST）与直接 drop 重开（WAL 重放），要求始终与常驻的 heap 库一致。该测试发现并修复了一个真实 bug（`cba658e`）：`LsmEngine::scan` 曾把 `snapshot()` 已排好的「新→旧」序列再 `reverse()`，导致跨表同键时**旧版本胜出**，表现为「先 flush 再 update」后新旧两版同时可见；已改为直接按新→旧归并，并在 `tests/lsm_engine.rs` 留聚焦回归用例。
...
**缓冲池命中统计与替换日志**：`PoolStats` 扩为 `{ hits, misses, evictions, dirty_evictions, resident, capacity }` 并提供 `hit_rate()`/`clean_evictions()`；新增 `CacheReporter` 接缝（生产 `StderrReporter`，测试注入 reporter 收集事件）与 `[observability]` 配置（`cache_stats`/`eviction_log`，默认关闭，行为与旧版逐位一致）。`cache_stats` 在每次 checkpoint（`flush_inner`）与 `BufferPool::drop` 输出快照；`eviction_log` 每次淘汰输出「文件、页、策略、脏位、驻留/容量、命中率」，淘汰事件先收集、**释放 `state` 锁后**再上报，避免持锁 I/O。`Database::open_with_config` 经 `BufferPool::new_with_observability` 接线；`tests/storage_buffer.rs` 断言干净/脏换出与开关关闭时静默，`tests/config.rs` 覆盖新节解析与严格模式。

**缓冲池静默丢写（2026-09-14 修复）**：`os_storage.md` §6.7 一直声称"命中/新建两条路径都在 `state` 锁内 pin"，但新建路径把这个 `frame` 交出了锁作用域，`PinnedFrame::new(frame)` 实际落在 `state` 锁**释放之后**：池满且其余帧全被 pin 时（4 帧池 + 4 线程的典型形态），这个 `pins == 0` 的新帧是 `choose_victim` 唯一能选中的对象，会被别的线程摘掉（干净帧不回写），本线程随后 pin 到一个**已脱离页表的孤儿帧**——它写下去的内容永远不回盘。是静默丢写，且不需要罕见时序：`tests/storage_buffer.rs` 的并发 hammer 用例（容量 4 / 4 线程 / 8 页原子自增）修复前连跑 60 次**红 18 次**，修复后 60 次全绿。修法是把 `PinnedFrame::new` 留在锁作用域内（`frame_for` 直接返回它）。不变式的准确表述：pin 的加一**属于"帧进入页表"这个临界区本身**，不是"在锁附近"。详见 `docs/os_storage.md` §6.7。
