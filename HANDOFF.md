...
> 最后更新：M22（UNION / UNION ALL）完成，291 tests 全绿、clippy 零警告。M23–M25 待做。
...
```powershell
cargo test                      # 全量回归（291 tests，20+ 个测试二进制）
...
进度：M20–M22 完成（291 tests）。接下来依次 M23 SQL 小补齐 → M24 元数据锁 → M25 索引消除排序。

提交基线：`83b2a23 feat: add UNION and UNION ALL`（HEAD）。
