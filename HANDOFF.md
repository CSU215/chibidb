...
> 最后更新：P5 算子覆盖单表/多表 SELECT、分组聚合、UNION；仅视图 FROM 与子查询内部仍走物化；361 tests 全绿、clippy 零警告。
...
| `exec/operator.rs` | 火山算子：`PhysicalOperator` + `ExecContext{db,session}` + `TableScan`/`IndexScan`/`ConstantScan`/`Filter`/`Project`/`Distinct`/`Sort`/`GroupBy`/`NestedLoopJoin`/`Union`/`Limit`；`build_select`/`build_set_op` 递归建树（视图 FROM 等未覆盖形态返回 `None` 回落物化）；`Database::collect_plan` 驱动 | |
...
提交基线：`9b164bd test: lock in operator plan coverage for representative selects`（HEAD）。
...
| P5 | 执行模型：基准 → 火山算子 → Chunk | 🟡 顶层 SELECT 全走算子（单/多表、分组聚合、UNION、子查询谓词）；视图 FROM 与子查询内部仍物化；Chunk 未做 |
