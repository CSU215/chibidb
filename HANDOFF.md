...
> 最后更新：P10.2b 完成（`BufferPool` 每帧页闩 + 元数据锁，方法改为 `&self`）；385 tests 全绿、clippy 零警告。
...
提交基线：`f56694b feat: make BufferPool thread-safe with per-frame latches`（HEAD）。
...
| P10 | 并发：`ThreadHandler`（per-connection/thread-pool）+ 去全局锁 + 可配置冲突策略（FCW/2PL） | 🟡 P10.1 完成：分库锁 + 去全局锁（`4984822`）+ `ThreadHandler` 双后端（`a1acc25`）；P10.2a 完成：只读 autocommit 走本地快照事务，不写 `next_trx_id`/`open_trxs`/`committed_trxs`（`879c39e`）；P10.2b 完成：`BufferPool` 元数据锁 + 每帧 `Mutex<PageData>`、`AtomicBool` 脏位，方法 `&self`（`f56694b`）；P10.2c/d 与 P10.3 FCW/2PL 待做 |
