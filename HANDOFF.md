...
> 最后更新：P2 三个接缝落地；P3 多库（目录管理 + `CREATE/DROP DATABASE`/`USE`）可用；336 tests 全绿、clippy 零警告。
...
| `instance.rs` | 单实例多库管理：数据根目录下 `chibi_meta/` + 每库一目录；`create/list/drop_database`、`database_mut` 懒打开缓存；`execute_with` 顶层入口（拦截 DB 级语句，其余路由到 current_db） | `Instance::open` / `execute_with` |
...
提交基线：`1642c56 feat: add Instance::execute_with with USE and session current database`（HEAD）。
...
| P3 | 单实例多库 + 系统元数据库 + 用户/权限 | 🟡 `Instance` 多库管理 + SQL `CREATE/DROP DATABASE`/`USE` + `Session.current_db`（`e99c5ef`、`3997571`、`1642c56`）；前端接 Instance、系统表/用户待做 |
