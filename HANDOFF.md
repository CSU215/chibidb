...
> 最后更新：P2 三接缝落地；P3 多库可用并接入前端（默认库 `main`）；337 tests 全绿、clippy 零警告。
...
提交基线：`6a101d5 test: cover default database auto-selection`（HEAD）。
...
| P3 | 单实例多库 + 系统元数据库 + 用户/权限 | 🟡 多库管理 + SQL `CREATE/DROP DATABASE`/`USE` + `Session.current_db` + 前端接 `Instance`（未选库自动用默认库 `main`）（`e99c5ef`…`cf6a4a9`）；系统表/用户待做 |
