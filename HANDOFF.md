...
> 最后更新：P5 算子覆盖单表 SELECT（scan/index/filter/project/distinct/sort/limit）+ 常数查询 + 分组/聚合（`GroupBy`）；359 tests 全绿、clippy 零警告。
...
| `exec/operator.rs` | 火山算子：`PhysicalOperator` + `ExecContext{db,session}` + `TableScan`/`IndexScan`/`ConstantScan`/`Filter`/`Project`/`Distinct`/`Sort`/`GroupBy`/`Limit`；`build_select` 递归建树（未覆盖形态返回 `None` 回落物化）；`Database::collect_plan` 驱动 | |
...
提交基线：`4a37501 feat: add GroupBy operator reusing the grouped-select logic`（HEAD）。
...
（P5 前物化基线分别为 31.1/23.8/18.9/27.2ms）。
...
| P5 | 执行模型：基准 → 火山算子 → Chunk | 🟡 算子覆盖：单表 SELECT（scan/index/filter/project/distinct/sort/group/limit）+ 常数查询；JOIN/UNION/子查询仍物化；Chunk 未做 |
