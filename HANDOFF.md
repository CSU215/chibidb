...
> 最后更新：P2 三个接缝（存储读/Protocol/Stage）落地；P3 起步（`Instance` 多库管理）；328 tests 全绿、clippy 零警告。
...
| `instance.rs` | 单实例多库管理：数据根目录下 `chibi_meta/` + 每库一目录；`create/list/drop_database`、`database_mut` 懒打开缓存 | `Instance::open` |
...
提交基线：`e99c5ef feat: add Instance managing multiple database directories`（HEAD）。
...
| P3 | 单实例多库 + 系统元数据库 + 用户/权限 | 🟡 `Instance` 多库目录管理落地（`e99c5ef`）；SQL `CREATE/DROP DATABASE`/`USE`、系统表/用户待做 |
