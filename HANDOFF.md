...
> 最后更新：P10 并发阶段完成（P10.2 库内读并发 + P10.3 可配置 FCW 冲突检测）；390 tests 全绿、clippy 零警告。
...
提交基线：`e068598 feat: first-committer-wins conflict detection with a config switch`（HEAD）。
...
| P10 | 并发：`ThreadHandler`（per-connection/thread-pool）+ 去全局锁 + 可配置冲突策略（FCW/2PL） | 🟡 P10.1：分库锁 + 去全局锁（`4984822`）+ `ThreadHandler` 双后端（`a1acc25`）。P10.2a：只读 autocommit 走本地快照事务，不写 `next_trx_id`/`open_trxs`/`committed_trxs`（`879c39e`）。P10.2b：`BufferPool` 元数据锁 + 每帧 `Mutex<PageData>`/`AtomicBool` 脏位，方法 `&self`（`f56694b`）。P10.2c：WAL 内部 `Mutex`（`f56c78d`）、Catalog `RwLock`（`9f8e801`）、事务簿记 `Atomic*`/`RwLock`（`64b52c1`）。P10.2d：`Database` 方法全 `&self`、读路径 `&Database`、`Instance` 每库 `RwLock<Database>`（读并发、写独占）（`2c70856`）。P10.3：`transaction.conflict = "fcw" | "2pl"`，FCW 提交时按 `prev_deleter`/当前标记检测写写冲突并回滚失败方（`e068598`）；2PL 仅接受配置、暂按非 FCW 路径运行（待实现锁等待/死锁检测） |
