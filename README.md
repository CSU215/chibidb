...
`storage.inline_lob_limit`、`wal.checkpoint_threshold`、`server.addr`、
...
的字符串在行编码时外存为 LOB 引用、解码时解析回字符串；版本被物理删除（回滚/vacuum）时回收其
LOB 文件（`LobReader` 支持流式读，查询路径暂整体物化；DROP TABLE 暂不回收该表 LOB）。
...
`cargo test` 跑 444 个测试，覆盖词法/语法/求值/LIKE/字符串函数/聚合/连接/子查询（含相关）/UNION/
