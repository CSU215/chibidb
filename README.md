...
LSM 侧构建中：写缓冲 `MemTable`（有序 + 墓碑 + 字节计数）与块格式
（`src/storage/lsm/`，varint 编码 + 前缀压缩块 + restart 数组）已就绪，
SSTable writer/reader、bloom、compaction 与 `CREATE TABLE ... ENGINE=lsm` 待接入。
...
`cargo test` 跑 405 个测试，覆盖词法/语法/求值/LIKE/字符串函数/聚合/连接/子查询（含相关）/UNION/
