//! Minimal C ABI for embedding chaoticdb (built as a `cdylib`), so non-Rust
//! frontends and the benchmark harness can call the engine in-process.
//!
//! Strings are NUL-terminated UTF-8. A handle comes from [`chaoticdb_open`] and
//! is released with [`chaoticdb_close`]; a result string comes from
//! [`chaoticdb_query`] and is released with [`chaoticdb_free`].
...
use chaoticdb::config::{Config, ExecutionMode, PageLayout};
use chaoticdb::sql::result;
use chaoticdb::Database;

/// Opaque handle to an open database.
pub struct ChaoticDb {
    inner: Database,
}
...
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_open(dir: *const c_char, mode: *const c_char) -> *mut ChaoticDb {
    ...
    match Database::open_with_config(Path::new(&dir), &config) {
        Ok(inner) => Box::into_raw(Box::new(ChaoticDb { inner })),
        Err(error) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_exec(db: *mut ChaoticDb, sql: *const c_char) -> i32 {
    if db.is_null() {
...
/// [`chaoticdb_free`]). On error returns `{"error":"..."}`.
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_query(db: *mut ChaoticDb, sql: *const c_char) -> *mut c_char {
...
/// Releases a string returned by [`chaoticdb_query`].
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_free(ptr: *mut c_char) {
    if !ptr.is_null() {
        drop(unsafe { CString::from_raw(ptr) });
    }
}

/// Closes a handle from [`chaoticdb_open`].
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_close(db: *mut ChaoticDb) {
    if !db.is_null() {
...
#[unsafe(no_mangle)]
pub extern "C" fn chaoticdb_last_error() -> *const c_char {
