...
///
/// The reference crate keeps this in `sql::result`; the target crate's
/// `sql::result` exposes only `ResultSet`, so the encoder lives here next to the
/// JSON reader it depends on.
