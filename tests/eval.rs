...
fn eval_first(sql: &str) -> Result<Value, Box<dyn std::error::Error>> {
    ...
    match &stmts[0] {
        chaoticdb::sql::ast::Stmt::Select(s) => match &s.items[0] {
            chaoticdb::sql::ast::SelectItem::Expr(e) => Ok(chaoticdb::exec::eval_const(e)?),
...
fn like_escape() {
    ...
    assert_eq!(val("select 'a!b' like 'a!!b' escape '!';"), Value::Bool(true));
    // without ESCAPE, the character is an ordinary one
...
fn string_functions() {
    ...
    assert_eq!(val("select length('h茅llo');"), Value::Int(5), "counts chars not bytes");
