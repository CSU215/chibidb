...
fn parse_select(sql: &str) -> Box<chaoticdb::sql::ast::SelectStmt> {
    match chaoticdb::sql::parser::parse(sql).unwrap().remove(0) {
        Stmt::Select(s) => s,
...
fn hash_join_handles_duplicate_keys() {
    ...
    let rs = db
        ...
        .unwrap();
    match &rs[0] {
        chaoticdb::ResultSet::Rows { rows, .. } => assert_eq!(
            rows,
            &vec![
...
fn dml_builds_operator_plans() {
    ...
    for sql in [
        ...
        let stmt = chaoticdb::sql::parser::parse(sql).unwrap().remove(0);
        assert!(build_statement(&db, &stmt).unwrap().is_some(), "{sql}");
    }

    // DDL is not an operator statement
    let ddl = chaoticdb::sql::parser::parse("create table u (id int);").unwrap().remove(0);
