use chaoticdb::sql::pipeline::{OptimizeStage, Pipeline, ResolveStage, SqlEvent, Stage};
use chaoticdb::{Database, Error, Result, ResultSet, Session};
...
fn one_stmt(sql: &str) -> chaoticdb::sql::ast::Stmt {
    chaoticdb::sql::parser::parse(sql).unwrap().remove(0)
}
...
fn pipeline_runs_stages_in_order() {
    ...
    pipeline.run(&db, &mut session, &mut event).unwrap();
...
fn pipeline_stops_on_stage_error() {
    ...
    assert!(pipeline.run(&db, &mut session, &mut event).is_err());
