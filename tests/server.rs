...
use chaoticdb::server::{serve, SharedInstance};
use chaoticdb::value::Value;
use chaoticdb::Result;
...
async fn exec_remote(stream: &mut TcpStream, sql: &str) -> Result<Vec<ResultSet>> {
    ...
    loop {
        ...
        match chaoticdb::wire::decode_frame(&body).unwrap() {
            chaoticdb::wire::Frame::Message(m) => out.push(ResultSet::Message(m)),
            chaoticdb::wire::Frame::Affected(n) => out.push(ResultSet::Affected(n)),
            chaoticdb::wire::Frame::Error(e) => failure = Some(chaoticdb::Error::Runtime(e)),
            chaoticdb::wire::Frame::Rows(rs) => out.push(rs),
            chaoticdb::wire::Frame::Done => {
                match failure {
                    Some(e) => return Err(e),
                    None => return Ok(out),
                }
